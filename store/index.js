...
function getBreadcrumbs(pageType, data) {
  const crumbs = []
  crumbs.push({
    title: 'Главная',
    url: '/',
...
export const actions = {
  ...
  async getCurrentCategory({ commit, dispatch }, { route }) {
    ...
    const crubms = getBreadcrumbs('category', category)
    await dispatch('setBreadcrumbs', crubms)
    await commit(
      'SET_CURRENT_CATEGORY',
      addProductsToCategory(products, productsImages, category)
    )
  },
  async getCurrentProduct({ commit, dispatch }, { route }) {
    ...
    const crubms = getBreadcrumbs('product', product)
    await dispatch('setBreadcrumbs', crubms)
